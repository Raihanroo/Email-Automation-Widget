...
/**
 * @testing-library/preact isn't paired with @testing-library/user-event
 * in this package, so form interactions go through fireEvent instead —
 * fireEvent.change for text inputs (userEvent.type isn't available) and
 * fireEvent.click for buttons/tabs. Behaviour asserted mirrors @eaw/react's
 * suite exactly; only the interaction mechanics differ.
 */
...
function uploadCsv(input: HTMLElement, file: File) {
...
describe("EmailAutomationWidget (dashboard mode)", () => {
  ...
  it("renders stat cards, recent mailbox, and recent activity once all three requests resolve", async () => {
    ...
    render(<EmailAutomationWidget mode="dashboard" baseURL="/api" />);
  ...
  it("shows an error message and calls onError when a dashboard request fails", async () => {
    ...
    const onError = vi.fn();
    render(<EmailAutomationWidget mode="dashboard" onError={onError} />);
...
describe("EmailAutomationWidget (composer mode)", () => {
  ...
  it("shows field-level validation errors after the user edits and leaves fields empty, without calling the network", async () => {
    ...
    typeInto(subjectInput, "x");
    typeInto(subjectInput, "");
  ...
  it("submits a valid form, calls onEmailSent, shows a success message, and resets the form", async () => {
    ...
    typeInto(screen.getByLabelText("To"), "user@example.com");
    typeInto(screen.getByLabelText("Subject"), "Hello");
    typeInto(screen.getByLabelText("Message"), "Test message");
    fireEvent.click(screen.getByRole("button", { name: /send/i }));
...
describe("EmailAutomationWidget (bulk mode)", () => {
  ...
  it("warns about invalid entries without blocking the valid ones", async () => {
    render(<EmailAutomationWidget mode="bulk" />);
    typeInto(screen.getByLabelText("Recipients"), "a@x.com, not-an-email");
    expect(await screen.findByText("1 valid recipient")).toBeInTheDocument();
    expect(
  ...
  it("blocks submit and shows errors when there are no valid recipients, subject, or body", async () => {
    ...
    render(<EmailAutomationWidget mode="bulk" />);
    fireEvent.click(screen.getByRole("button", { name: /send to all/i }));
  ...
  it("shows a server error and does not reset the form when the request fails", async () => {
    ...
    typeInto(screen.getByLabelText("Recipients"), "a@x.com");
    typeInto(screen.getByLabelText("Subject"), "Hello");
    typeInto(screen.getByLabelText("Message"), "Test message");
    fireEvent.click(screen.getByRole("button", { name: /send to all/i }));
  ...
  describe("recipient source toggle: paste vs CSV", () => {
    ...
    it("parses a valid CSV, showing the recipient count and detected columns", async () => {
      ...
      uploadCsv(screen.getByLabelText("CSV file"), makeCsvFile(csv));
    ...
    it("sends per-recipient placeholderData parsed from CSV columns", async () => {
      ...
      typeInto(screen.getByLabelText("Subject"), "Hi {{name}}");
      typeInto(screen.getByLabelText("Message"), "Welcome, {{name}}!");
      fireEvent.click(screen.getByRole("button", { name: /send to all/i }));
    ...
    it("blocks submit with a validation error when the CSV parse yields zero recipients", async () => {
      ...
      render(<EmailAutomationWidget mode="bulk" />);
      fireEvent.click(screen.getByRole("tab", { name: "Upload CSV" }));
      uploadCsv(screen.getByLabelText("CSV file"), makeCsvFile("name\nAlice"));
      typeInto(screen.getByLabelText("Subject"), "Hello");
      typeInto(screen.getByLabelText("Message"), "Test message");
      fireEvent.click(screen.getByRole("button", { name: /send to all/i }));
